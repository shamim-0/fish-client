...
const EventCategory = () => {
    ...
    const handleAddCategory = (e) => {
        ...
        fetch('http://localhost:5000/venue-category-add', {
            method: "POST",
            headers: {
    ...
    const handleDeleteCategory = (_id)=>{
        const url =  `http://localhost:5000/venue-category-delete?_id=${_id}`
        fetch(url, {
            method: "DELETE"
        })
        .then(res=> res.json())
    ...
    return (
        <div className='p-5'>
            <h2 className='text-xl font-medium my-5 text-center title'>Add Venue Category</h2>
            <div>
